...
class DeploymentCommand extends import_commander.Command {
  /**
   ...
   */
  addDeploymentSelectionOptions(action) {
    return this.addOption(
      new import_commander.Option("--url <url>").conflicts(["--prod", "--preview-name", "--deployment-name"]).hideHelp()
    ).addOption(new import_commander.Option("--admin-key <adminKey>").hideHelp()).addOption(
...
function deploymentClient(deploymentURL, onError) {
  ...
  (0, import_axios_retry.default)(client, {
    ...
    retryCondition: (error) => {
      ...
      return error.response?.status === 404 || import_axios_retry.default.isNetworkOrIdempotentRequestError(error);
    }
  });
  return client;
}
