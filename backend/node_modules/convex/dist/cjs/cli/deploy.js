...
const deploy = new import_commander.Command("deploy").summary("Deploy to your prod deployment").description(
...
).addOption(new import_commander.Option("--debug-bundle-path <path>").hideHelp()).addOption(new import_commander.Option("--debug").hideHelp()).addOption(new import_commander.Option("--admin-key <adminKey>").hideHelp()).addOption(new import_commander.Option("--url <url>").hideHelp()).addOption(new import_commander.Option("--log-deployment-name").hideHelp()).addOption(
  new import_commander.Option(
    "--preview-name <name>",
  ...
  async (cmdOptions) => {
    ...
    if (configuredDeployKey !== null && (0, import_deployment.deploymentTypeFromAdminKey)(configuredDeployKey) === "preview") {
  ...
  const data = await (0, import_utils.bigBrainAPI)({
    ...
    data: {
      projectSelection: await (0, import_api.projectSelection)(
        ctx,
        await (0, import_utils.getConfiguredDeployment)(ctx),
...
async function handleProduction(ctx, options) {
  ...
  const { adminKey, url, deploymentNames } = await (0, import_api.fetchDeploymentCredentialsWithinCurrentProject)(
    ctx,
    deploymentSelection
  );
  if (deploymentNames !== void 0 && deploymentNames.configured !== null) {
    const shouldPushToProd = deploymentNames.selected === deploymentNames.configured || (options.yes ?? await askToConfirmPush(
      ctx,
      {
