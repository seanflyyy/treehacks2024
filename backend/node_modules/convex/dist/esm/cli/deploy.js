...
export const deploy = new Command("deploy").summary("Deploy to your prod deployment").description(
...
).addOption(new Option("--debug-bundle-path <path>").hideHelp()).addOption(new Option("--debug").hideHelp()).addOption(new Option("--admin-key <adminKey>").hideHelp()).addOption(new Option("--url <url>").hideHelp()).addOption(new Option("--log-deployment-name").hideHelp()).addOption(
  new Option(
    "--preview-name <name>",
  ...
  async (cmdOptions) => {
    ...
    if (configuredDeployKey !== null && deploymentTypeFromAdminKey(configuredDeployKey) === "preview") {
...
async function handleProduction(ctx, options) {
  ...
  const { adminKey, url, deploymentNames } = await fetchDeploymentCredentialsWithinCurrentProject(
    ctx,
    deploymentSelection
  );
